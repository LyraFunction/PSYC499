...
data$party_diff <- data$Republican_Votes - data$Democratic_Votes
data$party_ratio <- data$Republican_Votes / data$Democratic_Votes

dataDem <- data |>
  filter(Winning_Party == 0)
...
#If you don't know how to use R Markdown, you can use the function "sink" to create a text file of your output
#This starts the sink
sink(file = "model_Dem.txt")

fit.growth_1<-growth(sem_model, data = dataDem, estimator = "ML", se = "bootstrap", missing = "ml", check.gradient = FALSE, control=list(iter.max=1000))
summary(fit.growth_1,fit.measures = TRUE,standardized=T, ci=TRUE)
parameterEstimates(fit.growth_1)
...
#If you don't know how to use R Markdown, you can use the function "sink" to create a text file of your output
#This starts the sink
sink(file = "model_Rep.txt")

fit.growth_1<-growth(sem_model, data = dataRep, estimator = "ML", se = "bootstrap", missing = "ml", check.gradient = FALSE, control=list(iter.max=1000))
summary(fit.growth_1,fit.measures = TRUE,standardized=T, ci=TRUE)
parameterEstimates(fit.growth_1)
